...
这个目录里每一条 `.md` 就是一页。七个子目录对应七个分类，**文件必须放在与其 `category` 一致的目录里**，否则构建失败。

```
content/
  symptoms/      ① 常见症状自查
  redflags/      ② 红旗警示（必须立即就医）
  firstaid/      ③ 急救技能（在救护车到达之前）
  diseases/      ④ 常见疾病常识
  medications/   ⑤ 用药与检查常识
  labs/          ⑥ 体检指标解读
  lifestyle/     ⑦ 养生
...
②与③是一对：**红旗警示回答「什么时候该叫急救」，急救技能回答「在急救到达之前怎么做」**。写的时候别把两者混在一起——前者要短、要能被记住，后者要具体、要能照着做。
...
| 分类 | 建议周期 | 理由 |
| --- | --- | --- |
| 红旗警示 / 急救技能 | 每年 | 指南本身变化慢，但一旦变化影响最大，不能漏 |
| 常见疾病 / 体检指标 / 用药与检查 | 每年 | 指南与切点更新较频繁 |
| 养生 | 每两年 | 生活方式证据变化最慢 |
...
`severity: emergency` 的条目，正文里**必须**有「何时必须就医」这一段，否则构建失败。校验器同时接受几个等价标题：`## 什么时候该问医生药师`、`## 什么时候该问医生`、`## 何时该问医生药师`、`## 何时该问医生` —— 药物速查类说「什么时候该问药师」比「何时必须就医」准确得多，不该为迁就校验器去写一句不自然的话。要加新的等价标题，记得同步改 `scripts/lib/content.mjs` 里的 `SECTION_ALIASES`。
