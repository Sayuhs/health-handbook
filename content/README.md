...
这个目录里每一条 `.md` 就是一页。八个子目录对应八个分类，**文件必须放在与其 `category` 一致的目录里**，否则构建失败。
...
`severity: emergency` 的条目，正文里**必须**有「何时必须就医」这一段，否则构建失败。校验器同时接受几个等价标题：`## 什么时候该问医生药师`、`## 什么时候该问医生或营养师`、`## 什么时候该问医生`、`## 何时该问医生药师`、`## 何时该问医生` —— 药物速查类说「什么时候该问药师」、食物选择类说「什么时候该问医生或营养师」，都比「何时必须就医」准确得多，不该为迁就校验器去写一句更窄的话。要加新的等价标题，记得同步改 `scripts/lib/content.mjs` 里的 `SECTION_ALIASES`。
